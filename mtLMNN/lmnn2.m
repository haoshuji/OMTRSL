...
if(size(x,1)>length(L)) error('x and L must have matching dimensions!\n');end;
 ...
 pars.valrand=1;
 pars.ntr = (1-pars.validation)*length(y);
...
%itr=1:(1-pars.validation)*length(y); ite=itr(end)+1:length(y);
itr=1:pars.ntr; ite=pars.ntr+1:length(y);
xv=x(:,ite);
yv=y(:,ite);
x=x(:,itr);
y=y(itr);
...
if isfield(pars,'L_for_nn')
  [gen,NN]=getGenLS(pars.L_for_nn*x,y,Kg,pars);
else
...
for iter=1:pars.maxiter
    ...
    if(iter>10)
        if (max(abs(diff(obj(iter-10:iter))))<pars.thresho*obj(iter)  | stepsize<pars.thresha)
    ...
    if(pars.tempid>=0 && mod(iter,50)==0) 
      time=toc;
      % save(tempname,'L','iter','obj','pars','time','verify');
    end;
